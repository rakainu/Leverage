...
class ExecutionConfig:
    """How orders are placed. `paper` = lighter.PaperClient simulation (legacy, what
    the scalper/squeeze bridges use). `real` = lighter.SignerClient places REAL signed
    orders (the Booster testnet book) while market data still comes from PaperClient.
    Credentials NEVER live in yaml: account/key indices may, but the private key is
    read from env LIGHTER_PRIVATE_KEY only."""
...
def load_config(path: str | Path) -> BridgeConfig:
    ...
    # private key NEVER from yaml — env only, so it's never committed.
    execution.private_key = os.environ.get("LIGHTER_PRIVATE_KEY", "")
    if execution.mode == "real" and not execution.private_key:
        raise ValueError("execution.mode 'real' requires LIGHTER_PRIVATE_KEY in env")
