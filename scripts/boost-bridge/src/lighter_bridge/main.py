...
from .signer_executor import SignerExecutor
from . import notify
from . import sizing
from .signals import (
...
class Bridge:
    def __init__(self, cfg: BridgeConfig):
        ...
        self.signer: "lighter.SignerClient | None" = None   # real-order client (execution.mode=='real')
        self.executor: "PaperExecutor | SignerExecutor | None" = None
    ...
    async def start(self):
        ...
        if self.cfg.execution.mode == "real":
            ex = self.cfg.execution
            self.signer = lighter.SignerClient(
                url=self.cfg.host, account_index=ex.account_index,
                api_private_keys={ex.key_index: ex.private_key})
            chk = self.signer.check_client()
            if chk is not None:
                raise RuntimeError(f"SignerClient validation failed: {chk}")
            self.executor = SignerExecutor(self.paper, self.signer, self.api,
                                           ex.account_index, exec_symbols,
                                           poll_s=self.cfg.loop.position_check_interval_s)
            log.info("EXECUTION: REAL orders via SignerClient (account_index=%d) — testnet book",
                     ex.account_index)
        else:
            self.executor = PaperExecutor(self.paper, exec_symbols)
            log.info("EXECUTION: PAPER (lighter.PaperClient simulation)")
    ...
    async def _on_new_bar_regime(self, symbol: str, df: pd.DataFrame):
        ...
        if pe is not None:
            ...
            if not self._entries_allowed(symbol):
                log.info("%s: entries OFF — cancelling resting maker-limit @ %.4f",
                         symbol, pe["limit_px"])
                if self._is_real():
                    await self.executor.cancel_entry(symbol)
                self.pending_entry.pop(symbol, None)
                return
            pe["bars"] += 1
            if self._is_real():
                # Real maker fills are detected on the fast position loop (poll_entry_fill),
                # not by replaying the bar. Here we only EXPIRE the resting order.
                if not self.executor.has_pending(symbol):
                    self.pending_entry.pop(symbol, None)   # filled (finalized by fast loop) or gone
                elif pe["bars"] >= rg.entry_valid_bars:
                    log.info("%s: regime %s real limit @ %.6f unfilled after %d bars — cancel",
        ...
        if self._is_real():
            if base_amount <= 0:
                return
            armed = await self.executor.arm_maker_entry(symbol, side, base_amount, lv["limit_px"])
            if not armed:
                return  # real maker order rejected (e.g. would cross / below min) — skip this bar
    ...
    async def _finalize_regime_real(self, symbol: str, pos, fq, pe: dict):
        ...
        SL/TP off the real fill price, log the trade + fill-quality telemetry, notify."""
        if pe is None:
            return
        side = pos.side
        now = datetime.now(timezone.utc).isoformat()
        ...
        trade_id = self.db.log_trade(
            ...
            opened_at=now, bar_time_open=str(pe["signal_ts"]), adx_at_entry=0.0)
        self.trade_ids[symbol] = trade_id
        self.db.log_fill_quality(
            trade_id=trade_id, symbol=symbol, side=side, requested_px=fq.requested_price,
            mark_px=fq.mark_at_request, filled_px=fq.filled_price, maker=fq.maker,
            slippage_bps=fq.slippage_bps, wait_s=fq.wait_seconds, ts=now)
        log.info("%s: REGIME(real) %s FILLED entry=%.6f SL=%.6f TP=%.6f notional=$%.0f "
                 "(%.1f bps adverse, %.0fs to fill)", symbol, side.upper(), pos.entry_price,
                 pos.sl_price, pos.tp_price, pos.notional, fq.slippage_bps, fq.wait_seconds)
        self.db.log_signal(symbol=symbol, side=side, bar_time=str(pe["signal_ts"]),
                           outcome="fired", detected_at=now)
        if self.cfg.notify.open:
            asyncio.create_task(notify.notify_open(pos))
    ...
    async def position_check_loop(self):
        ...
        while not self._stopped:
            # Real maker entries fill continuously on-chain — detect fills here (fast)
            # rather than on bar close, so SL/TP management starts promptly.
            if self._is_real():
                for sym in list(self.pending_entry.keys()):
                    if self.executor.has_pending(sym):
                        res = await self.executor.poll_entry_fill(sym)
                        if res is not None:
                            op, fq = res
                            await self._finalize_regime_real(sym, op, fq,
                                                             self.pending_entry.get(sym))
