...
CREATE TABLE IF NOT EXISTS fill_quality (
    ...
    symbol        TEXT,
    side          TEXT,
    requested_px  REAL,    -- the maker limit we posted
    mark_px       REAL,    -- order-book mid when armed
    filled_px     REAL,    -- realized fill (avg entry)
    maker         INTEGER, -- 1 = post-only maker fill
    slippage_bps  REAL,    -- adverse (filled - requested) in bps
    wait_s        REAL     -- seconds resting until filled
);
"""
...
class TradeLogDB:
    ...
    def log_fill_quality(self, *, trade_id, symbol, side, requested_px, mark_px,
                         filled_px, maker, slippage_bps, wait_s, ts):
        self.conn.execute(
            "INSERT INTO fill_quality (trade_id, ts, symbol, side, requested_px, mark_px, "
            "filled_px, maker, slippage_bps, wait_s) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (trade_id, ts, symbol, side, requested_px, mark_px, filled_px,
             1 if maker else 0, slippage_bps, wait_s))
        self.conn.commit()
