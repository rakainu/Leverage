...
class OpenPosition:
    ...
    notional: float = 0.0
    sl_price: float = 0.0     # fixed ATR stop, set at entry
    tp_price: float = 0.0     # fixed ATR target, set at entry
    atr_at_entry: float = 0.0
    # legacy fields kept so restore/db code that references them stays happy
    state: int = 0
...
class PaperExecutor:
    ...
    async def open_position(self, symbol: str, side: str) -> Optional[OpenPosition]:
