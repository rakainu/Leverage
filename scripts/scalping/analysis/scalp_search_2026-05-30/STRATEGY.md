...
4. **The one edge that GENERALIZES is regime-gated mean reversion** — fade
   ...
   trend**, across a 5-coin basket. After exit-geometry tuning: **PF 1.49, 89% WR,
   192 trades/wk**, profitable on all 5 coins (PF ≥1.33), OOS 1.42, 4/4 walk-forward
   (1.42–1.63), survives 2× slippage (1.43). This is the recommended candidate.
...
- **Rule:** On 15m, trend = sign of EMA(200) slope over 20 bars. Compute z-score
  ...
  z ≥ +1.5 → short.** Maker limit entry 0.25·ATR beyond close. Hard stop **2.0·ATR**.
  TP = **0.3 × (distance to VWAP)**, maker. Time stop 12 bars (3h). Both sides.
- **Coins/tf:** SOL+ETH+ZEC+HYPE+BTC, 15m, basket (one position per coin).
- **Frequency:** **192 trades/wk pooled (~27/day)**; 37–40/wk per coin.
- **Win rate:** **89%** (88–90% every coin).
- **Profit factor:** **1.49** pooled (per coin 1.33–1.74).
- **Expectancy:** +0.061%/trade after slippage; avg win +0.21%, avg loss −1.08%.
- **Avg hold:** 14 min.
- **Max DD:** **$54** on $250 notional @10x over 26 wk.
- **OOS/WF:** IS PF 1.52 / **OOS PF 1.42**; folds 1.42 / 1.46 / 1.63 / 1.49 (4/4 +).
- **Slippage:** 2× (0.10%) → PF **1.43** (barely dents).
- **Best leverage/notional:** **10x ONLY (0 liquidations; maxMAE 7.3% < liqDist 9.95%).**
  20x → 5 liq /4932 (the 2.0·ATR stop + ~7% MAE tails breach liqDist 4.98%); 30x →
  24 liq. $250 @10x → **+$756 / 26 wk**.
- **Why it passes:** generalizes across all coins (every coin PF ≥1.33), robust
  OOS+WF+slippage, true high-frequency scalp cadence, 89% WR. Clears the 1.40
  *preferred* bar, not just the 1.25 minimum.
- **Risk to watch:** wide-stop/tight-TP profile → breakeven WR ≈ **84%** (loss is
  ~5× win), ~4.7 pts of cushion above the 88.5% actual. Rare losses run −3% to −5%.
  A sustained chop/whipsaw regime that decays WR below ~85% flips it negative —
  hence the WR kill-switch and 10x-only leverage cap.

**Tuning note — exit management tested and rejected (honest negative result).**
Breakeven-trail and partial-TP/runner were both implemented in the engine
(regression-clean) and swept. Neither helped: BE-trail is inert (holds ~1 bar; TP
≪ stop, so a trade hits TP before it can move +1R to arm BE), and scale-out HURTS
(PF→1.07 — a mean-reversion move is done at VWAP, no trend for a runner to harvest).
The PF lift from 1.22→1.49 came from **stop/TP geometry** (stop 1.5→2.0·ATR,
TP 0.4→0.3×) — bank the quick reversion, give the trade room to resolve. Verified
strictly better than the old config on PF, WR, breadth, and drawdown at equal freq.
...
stop             : 2.0*ATR(14) hard (taker)
take profit      : 0.3 * (entry distance to VWAP), maker limit
time stop        : 12 bars (3h) -> market close
sizing           : fixed $200-250 notional per position
leverage         : 10x ONLY (0 liquidations; the 2.0*ATR stop breaches liqDist at 20x+)
```

**Kill-switch rules (critical — the edge is thin above breakeven WR ≈ 84%):**
- Rolling 100-trade win rate **< 85%** → halt (approaching breakeven, edge gone).
- Rolling 30-day PF **< 1.15** → halt and review.
- Any single coin's 50-trade WR **< 84%** → drop that coin from the basket.
- Account drawdown **> 6%** → halt (backtest 26-wk maxDD was ~$54 on $250@10x;
  >6% means regime break).
- Daily loss **> 3R** → stop for the day.

**What to optimize next (in priority order):**
1. ~~Lift PF above 1.25~~ **DONE** — stop/TP geometry (2.0·ATR / 0.3×) → PF 1.49.
   Breakeven-trail & partial-TP tested and rejected (don't fit a fast small-TP MR
   scalp; see tuning note above).
2. **Widen the WR cushion** — breakeven WR is 84% vs 88.5% actual. Test a light
   entry-quality filter (skip entries when VWAP-band width is abnormally low =
   chop) to lift WR without killing frequency. ADX gating did NOT help.
